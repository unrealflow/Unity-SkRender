Shader "SkRender/HDLit"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            float3 noise_normal(float3 normal, float3 p,float t,float a)
            {
                ...
                float peak = a;
                t = t * (1.0 - peak) + peak;
                return normalize(lerp(p, normal, t));
            }
            ...
            void ClosestHitShader(inout RP rp : SV_RayPayload, AttributeData attribs: SV_IntersectionAttributes)
            {
                ...
                float3 color=0.0;
                SkMat m;
                float3 texColor=_BaseColorMap.SampleLevel(s_linear_clamp_sampler,v.uv,0.0).xyz;
                m.baseColor=_BaseColor.xyz*texColor;
                m.metallic=_Metallic;
                m.roughness=1.0-_Smoothness;
                uint shadowFlags=RAY_FLAG_SKIP_CLOSEST_HIT_SHADER;

                RayDesc rayDescriptor;
                rayDescriptor.Origin = v.position;
                rayDescriptor.TMin = 1e-5f;
                rayDescriptor.TMax = _FarClip;

                float3 noise=GetRandomInUnitSphere(rp.states);

                float3 N=v.normal;
                // float3 N=v.normal;
                float3 V=-WorldRayDirection();
                float3 F0 =0.04;
                F0=lerp(F0, m.baseColor,m.metallic);
                rp.kS=F_Schlick(dot(V,N),F0);
                rp.pos=v.position;
                for(int i=0;i<MAX_LIGHTS;i++)
                {
                    Light l=GetLight(i,v.position,noise);
                    if(l.tag>0)
                    {
                        float3 kS=0.0;
                        ShadowRP s_rp;
                        s_rp.shadowed=true;
                        rayDescriptor.Direction=l.dir;
                        TraceRay(_RTAS,shadowFlags,0xff,0,1,1,rayDescriptor,s_rp);
                        if(!s_rp.shadowed)
                        {
                            color+=l.color*BRDF(m,l.dir,V,N);
                        }
                    }
                }
